...
void set_text_arr(const string str1, const string str2, const string str3, const string str4, RenderWindow& window, text_class text_box[]) {
...
bool check_mouse(RenderWindow& window, int startX, int startY, int n, int tilesize)
{
        ...
        RenderWindow& window)
{
    switch (num_anim) {
    case 1:
		set_text_arr("Greatings Anon!", "I am the computer Goddess", "and I bless this PC", "", window, text_box);
        break;
	case 2:
		set_text_arr("And I give you this world.", "Give him a life!", "", "", window, text_box);
		break;
	case 3:
		goddess.set_anim_frame(522, 0, 522, 919, 0);
		set_text_arr("Life is wonderful, isn't it? ^^", "", "", "", window, text_box);
		break;
	case 4:
		goddess.set_anim_frame(522, 0, 700, 919, 4);
		set_text_arr("Destroy your whole life?", "How cruel ...", "", "", window, text_box);
		break;
	case 5:
		goddess.set_anim_frame(522, 0, 522, 919, 0);
		set_text_arr("It looks like your life does", "success!~", "", "", window, text_box);
		break;
	case 6:
	...
	case 7:
		...
		set_text_arr("It looks like you gave the world", "too little life...", "", "", window, text_box);
		break;
	case 8:
		goddess.set_anim_frame(522, 0, 522, 919, 3);
		set_text_arr("The stagnation of the world", "is the same as death...", "", "", window, text_box);
		break;
	case 9:
		goddess.set_anim_frame(522, 0, 522, 919, 1);
		set_text_arr("Enter - start, S - stop, Space - rand", "Q - line, R - reset", "Right - +size Left - -size", "Up - +speed, Down - -speed", window, text_box);
		break;
	case 10:
		goddess.set_anim_frame(522, 0, 522, 919, 0);
		set_text_arr("More or less, it does not matter", "", "", "", window, text_box);
		break;
	case 11:
		goddess.set_anim_frame(522, 0, 522, 919, 0);
		set_text_arr("Wow, this may take some time", "", "", "", window, text_box);
		break;
    }
}
int main()
{
	...
	int n = 400, day_num = 0, coord_num = 10, delay_t = 50, startX = 700, startY = 50, tilesize = 8, num_scene = 2;
	int pos_j, pos_i;
	string* coord_world;
